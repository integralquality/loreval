...
export default function SharedLevelPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen graph-paper">
  ...
  if (!levelData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen graph-paper text-center px-6">
        ...
        <Link to="/browse" className="font-mono text-orange-600 hover:text-orange-500 text-sm">
          browse levels
        </Link>
      </div>
    );
  ...
  if (!parsed) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen graph-paper text-center px-6">
        <h1 className="text-2xl font-bold text-zinc-900 mb-3">Failed to load level</h1>
        <Link to="/browse" className="font-mono text-orange-600 hover:text-orange-500 text-sm">
          browse levels
        </Link>
      </div>
  ...
  return (
    <div className="relative">
      ...
      {/* Floating info bar */}
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 px-5 py-2.5 bg-paper border-2 border-zinc-900 rounded-full shadow-xl">
        <span className="text-sm font-bold text-zinc-900">{levelData.name}</span>
