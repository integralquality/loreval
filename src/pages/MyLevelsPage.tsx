    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-zinc-900">My Levels</h1>
            <p className="text-zinc-500 mt-1">{levels.length} level{levels.length !== 1 ? 's' : ''}</p>
          </div>
          <Link
            to="/designer"
            className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 hover:bg-zinc-700 text-paper text-sm font-semibold rounded transition-colors"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {levels.map(level => {
              ...
              return (
                <div
                  key={level.id}
                  className="bg-white/60 border border-zinc-900/15 rounded-lg overflow-hidden hover:border-zinc-900/40 transition-colors"
                >
                  ...
                  <div className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <h3 className="text-zinc-900 font-bold truncate">{level.name}</h3>
                      <span className={`flex items-center gap-1 font-mono text-[11px] px-2 py-0.5 rounded ${
                        level.is_published
                          ...
                          : 'bg-zinc-900/5 text-zinc-500 border border-zinc-900/10'
                      }`}>
                    ...
                    <div className="flex gap-2 pt-1">
                      <Link
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 py-1.5 border border-zinc-900/20 hover:border-zinc-900 text-zinc-700 text-xs rounded transition-colors"
                      >
                        Edit
                      </Link>
                      <button
                      ...
                      {level.is_published && (
                        <button
                          ...
                          className="flex items-center justify-center gap-1 px-2 py-1.5 border border-zinc-900/20 hover:border-zinc-900 text-zinc-700 text-xs rounded transition-colors"
                      ...
                      {level.is_published && (
                        <Link
                          ...
                          className="flex items-center justify-center px-2 py-1.5 border border-zinc-900/20 hover:border-zinc-900 text-zinc-700 text-xs rounded transition-colors"
                          title="Play"
                        >
                      ...
                      <button
                        ...
                        className="flex items-center justify-center px-2 py-1.5 border border-zinc-900/20 hover:border-red-600 text-zinc-400 hover:text-red-600 text-xs rounded transition-colors"
                        title="Delete"
                      >
                        <Trash2 size={12} />
                      </button>
